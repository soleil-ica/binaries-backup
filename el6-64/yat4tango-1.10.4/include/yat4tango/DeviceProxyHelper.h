...
#define FULL_CMD_NAME(C) \
  tango_proxy()->dev_name() + "::" + C
//---------------------------------------------------------------------------
#define FULL_ATTR_NAME(A) \
  tango_proxy()->dev_name() + "/" + A
...
public:
  ...
  //---------------------------------------------------------------------------
  void command (const std::string& cmd_name, int num_retries = 1)
  {
    if ( device_proxy_ )
    {
      _TANGO_DEVICE_RETRY
      (
        tango_proxy()->command_inout(const_cast<std::string&>(cmd_name)),
  ...
  //---------------------------------------------------------------------------
  Tango::DeviceData command_inout (const std::string& cmd_name, Tango::DeviceData& argin, int num_retries = 1)
  {
    ...
    if ( device_proxy_ )
    {
      _TANGO_DEVICE_RETRY
      (
        argout = tango_proxy()->command_inout(const_cast<std::string&>(cmd_name), argin),
        std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
                      ...
                      int num_retries = 1)
  {
    if ( device_proxy_ )
    {
      ...
      _TANGO_DEVICE_RETRY
      (
        dd_out_ = tango_proxy()->command_inout(const_cast<std::string&>(cmd_name), dd_in),
        std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
        "yat4tango::DeviceProxyHelper::command_inout",
        num_retries
      );
  ...
  //---------------------------------------------------------------------------
  void command_in (const std::string& cmd_name, const Tango::DeviceData& argin, int num_retries = 1)
  {
    if ( device_proxy_ )
    {
      _TANGO_DEVICE_RETRY
      (
        tango_proxy()->command_inout(const_cast<std::string&>(cmd_name), const_cast<Tango::DeviceData&>(argin)),
        std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
                   ...
                   int num_retries = 1)
  {
    if ( device_proxy_ )
    {
      ...
      _TANGO_DEVICE_RETRY
      (
        tango_proxy()->command_inout(const_cast<std::string&>(cmd_name), dd_in),
        std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
        "yat4tango::DeviceProxyHelper::command_in",
        num_retries
      );
  ...
  //---------------------------------------------------------------------------
  Tango::DeviceData command_out (const std::string& cmd_name,  int num_retries = 1)
  {
    Tango::DeviceData argout;
    if ( device_proxy_ )
    {
      _TANGO_DEVICE_RETRY
      (
        argout = tango_proxy()->command_inout(const_cast<std::string&>(cmd_name)),
        std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
                    ...
                    int num_retries = 1)
  {
    if ( device_proxy_ )
    {
      _TANGO_DEVICE_RETRY
      (
        dd_out_ = tango_proxy()->command_inout(const_cast<std::string&>(cmd_name)),
        std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
        "yat4tango::DeviceProxyHelper::command_out",
        num_retries
      );
...
public:
  ...
  //---------------------------------------------------------------------------
  typedef std::vector<Tango::DeviceAttribute> AttributeSnapshot;
  typedef std::vector<AttributeSnapshot*> AttributeSnapshots;
                        ...
                        int num_retries = 1)
  {
    if ( device_proxy_ )
    {
      ...
      _TANGO_DEVICE_RETRY
      (
        tango_proxy()->write_attribute(da),
                       ...
                       int num_retries = 1)
  {
    if ( device_proxy_ )
    {
      _TANGO_DEVICE_RETRY
      (
        m_da_out = tango_proxy()->read_attribute(const_cast<std::string&>(attr_name)),
  ...
  //  AttributeHelper::read_attribute
  //---------------------------------------------------------------------------
  Tango::DeviceAttribute read_attribute (const std::string& attr_name, int num_retries = 1)
  {
    Tango::DeviceAttribute da;
    if ( device_proxy_ )
    {
      _TANGO_DEVICE_RETRY
      (
        da = tango_proxy()->read_attribute(const_cast<std::string&>(attr_name)),
        std::string("read_attribute on ") + FULL_ATTR_NAME(attr_name),
        "yat4tango::DeviceProxyHelper::read_attribute",
        num_retries
      );
  ...
  //  AttributeHelper::write_attribute
  //---------------------------------------------------------------------------
  void write_attribute (const Tango::DeviceAttribute& da, int num_retries = 1)
  {
    if ( device_proxy_ )
    {
      _TANGO_DEVICE_RETRY
      (
        tango_proxy()->write_attribute(const_cast<Tango::DeviceAttribute&>(da)),
  ...
  //---------------------------------------------------------------------------
  //  AttributeHelper::snapshot (for backward compatibility)
  //---------------------------------------------------------------------------
  inline void snapshot (const std::string& attribute_name, bool throw_exception_on_error = true)
  {
    snapshot_attribute(attribute_name, throw_exception_on_error);
  }

  //---------------------------------------------------------------------------
  //  AttributeHelper::snapshot (for backward compatibility)
  //---------------------------------------------------------------------------
  inline void snapshot (const std::vector<std::string>& attributes_list, bool throw_exception_on_error = true)
  {
    snapshot_attributes(attributes_list, throw_exception_on_error);
  }

  //---------------------------------------------------------------------------
  //  AttributeHelper::snapshot
  //---------------------------------------------------------------------------
  void snapshot_attribute (const std::string& attribute_name, bool throw_exception_on_error = true)
  {
  ...
  //  AttributeHelper::snapshot
  //---------------------------------------------------------------------------
  void snapshot_attributes (const std::vector<std::string>& attributes_list, bool throw_exception_on_error = true)
  {
    ...
    try
    {
      try
      {
        ...
        m_attr_snapshots.push_back(vda);
      }
      _HANDLE_TANGO_DEVICE_EXCEPTION("snapshot_attributes", "yat4tango::DeviceProxyHelper::snapshot_attributes");
    }
    catch (...)
    {
  ...
  {
    for ( size_t i = 0; i < m_attr_snapshots.size(); i++ )
    {
      AttributeSnapshot& s = *m_attr_snapshots[i];
  ...
  //---------------------------------------------------------------------------
  //  AttributeHelper::restore_attributes_snapshot
  //---------------------------------------------------------------------------
  void restore_attributes_snapshot (bool throw_exception_on_error = true)
  {
    if ( m_attr_snapshots.empty() )
      return;
    
    int tmo = tango_proxy()->get_timeout_millis();
      
    tango_proxy()->set_timeout_millis(10000);

    //------------------------------------------------------------------------------------------------
    //- NL-SOLEIL: 10/15/2016
    //------------------------------------------------------------------------------------------------
    //- please don't change the following implementation trying to optimize it using a single call 
    //- to <write_attributeS> instead of several call to <write_attribute> - this is NOT eaquivalent 
    //- at all on some devices (the beahavior is purely implementation dependent) - this is especially 
    //- true for those creating/removing dynamic attributes when the value of a given attribute change
    //------------------------------------------------------------------------------------------------
    try
    {
      try
      {
        for ( size_t i = 0; i < m_attr_snapshots.size(); i++ )
        {
          ...
          for ( size_t j = 0; j < s.size(); j++ )
          {
            tango_proxy()->write_attribute(s[j]);
          }
        }
      }
      _HANDLE_TANGO_DEVICE_EXCEPTION("restore_attributes_snapshot", "yat4tango::DeviceProxyHelper::restore");
    }
    catch (...)
    {
      tango_proxy()->set_timeout_millis(tmo);
  ...
  //---------------------------------------------------------------------------
  //  AttributeHelper::reset_attribute_snapshots
  //---------------------------------------------------------------------------
  void reset_attribute_snapshots ()
  {
    //- avoid memory leaks...
    for ( size_t i = 0; i < m_attr_snapshots.size(); i++ )
      delete m_attr_snapshots[i];
    m_attr_snapshots.clear();
  }
...
private:
  ...
  //- device snapshot  
  AttributeSnapshots m_attr_snapshots;
};
...
private:
  ...
  void pulser_cb(yat::Thread::IOArg)
  {
    ...
    if( (config_ptr_->state == ds && !config_ptr_->wait_end) ||
        (config_ptr_->state != ds && config_ptr_->wait_end)
      )
    {
  ...
  DeviceProxyHelperStateNotification notification(Tango::DevState ds)
  {
    ...
    notif.device_name = tango_proxy()->dev_name();
...
//=============================================================================
class DeviceProxyHelper : public virtual CommandInOutHelper, public virtual AttributeHelper, public virtual PolledStateHelper
{
public:
  //---------------------------------------------------------------------------
  // snapshots restore order
  //---------------------------------------------------------------------------
  typedef enum
  {
    ATTR_FIRST = 0,
    PROP_FIRST
  } RestoreSnapshotsOrder;

  //---------------------------------------------------------------------------
  // snapshot of device properties
  //---------------------------------------------------------------------------
  typedef std::map<std::string, std::vector<std::string> > PropertiesSnapshot;
  typedef PropertiesSnapshot::iterator PropertiesSnapshotIterator;
  typedef PropertiesSnapshot::const_iterator PropertiesSnapshotConstIterator;
  ...
  //  DeviceProxyHelper::state
  //---------------------------------------------------------------------------
  Tango::DevState state (int num_retries = 1)
  {
    Tango::DevState ds = Tango::UNKNOWN;
    if ( device_proxy_ )
    {
  ...
  //---------------------------------------------------------------------------
  std::string status (int num_retries = 1)
  {
  ...
  //---------------------------------------------------------------------------
  //  DeviceProxyHelper::snapshot_property
  //---------------------------------------------------------------------------
  void snapshot_property (const std::string& property_name, bool throw_exception_on_error = true)
  {
    std::vector<std::string>  pl(1, property_name);
    snapshot_properties(pl, throw_exception_on_error);
  }
  ...
  //---------------------------------------------------------------------------
  template <typename C> 
  bool snapshot_property (const std::string& property_name, const C& expected_property_value, bool throw_exception_on_error = true)
  {
    bool property_change_required = false;
    try
    {
      try
      {
        Tango::DbData dbds;
        tango_proxy()->get_property(const_cast<std::string&>(property_name), dbds);
        if ( dbds.empty() )
          return false;

        std::string current_property_str_value;
        dbds[0] >> current_property_str_value;

        yat::OSStream oss; 
        oss << expected_property_value;
        std::string expected_property_str_value = oss.str();

        std::transform(current_property_str_value.begin(), current_property_str_value.end(), current_property_str_value.begin(), ::tolower);
        std::transform(expected_property_str_value.begin(), expected_property_str_value.end(), expected_property_str_value.begin(), ::tolower);

        if ( current_property_str_value != expected_property_str_value )
        {
          std::vector<std::string> pv;
          dbds[0] >> pv;
          m_prop_snapshots[property_name] = pv;
          property_change_required = true;
        }
      }
      _HANDLE_TANGO_DEVICE_EXCEPTION("snapshot_property", "yat4tango::DeviceProxyHelper::snapshot_property");
    }
    catch (...)
    {
      if ( throw_exception_on_error ) 
    ...
    return property_change_required;
  }
  
  //---------------------------------------------------------------------------
  //  DeviceProxyHelper::snapshot_properties
  //---------------------------------------------------------------------------
  void snapshot_properties (const std::vector<std::string>& properties_list, bool throw_exception_on_error = true)
  {
    ...
    try
    {
      ...
      {
        Tango::DbData dbds;
        tango_proxy()->get_property(const_cast<std::vector<std::string>&>(properties_list), dbds);
        for ( size_t i = 0; i < dbds.size(); i++ )
        {
          std::vector<std::string> pv;
          dbds[i] >> pv;
          m_prop_snapshots[dbds[i].name] = pv;
        }
      }
      _HANDLE_TANGO_DEVICE_EXCEPTION("snapshot_properties", "yat4tango::DeviceProxyHelper::snapshot_properties");
    }
    catch (...)
    {
      if ( throw_exception_on_error ) 
  ...
  //---------------------------------------------------------------------------
  //  DeviceProxyHelper::put_property
  //---------------------------------------------------------------------------
  template <typename C> 
  void put_property (const std::string& property_name, const C& new_property_value, bool throw_exception_on_error = true)
  {
    try
    {
      try
      {
        yat::OSStream oss; 
        oss << new_property_value;
        std::string new_property_str_value = std::string(oss.str());
        Tango::DbDatum dbd(property_name);
        dbd << new_property_str_value;
        Tango::DbData dbds;
        dbds.push_back(dbd);
        tango_proxy()->put_property(dbds);
      }
      _HANDLE_TANGO_DEVICE_EXCEPTION("put_property", "yat4tango::DeviceProxyHelper::put_property");
    }
    catch (...)
    {
      if ( throw_exception_on_error ) 
  ...
  //---------------------------------------------------------------------------
  void restore_properties_snapshot (Tango::DevState expected_post_init_state, std::size_t tmo_sec, bool throw_exception_on_error = true)
  {
    if ( m_prop_snapshots.empty() )
      return;
    ...
    try
    {
      ...
      {
        Tango::DbData dbds;
        PropertiesSnapshotIterator it = m_prop_snapshots.begin();
        for ( ; it != m_prop_snapshots.end(); ++it )
        {
          Tango::DbDatum dbd(it->first);
          dbd << it->second;
          dbds.push_back(dbd);
        }
        tango_proxy()->put_property(dbds);
        init(expected_post_init_state, tmo_sec);
      }
      _HANDLE_TANGO_DEVICE_EXCEPTION("restore_properties_snapshot", "yat4tango::DeviceProxyHelper::restore_properties_snapshot");
    }
    catch (...)
    {
      if ( throw_exception_on_error ) 
  ...
  //---------------------------------------------------------------------------
  //  DeviceProxyHelper::restore
  //---------------------------------------------------------------------------
  void restore (Tango::DevState expected_post_init_state = Tango::UNKNOWN, std::size_t tmo_sec = 10, const RestoreSnapshotsOrder o = PROP_FIRST)
  {
    if ( PROP_FIRST == o ) 
    {
      restore_properties_snapshot(expected_post_init_state, tmo_sec);
      restore_attributes_snapshot();
    }
    else
    {
      restore_attributes_snapshot();
  ...
  //---------------------------------------------------------------------------
  //  DeviceProxyHelper::init
  //---------------------------------------------------------------------------
  void init (Tango::DevState expected_post_init_state, std::size_t tmo_sec)
  {
    tango_proxy()->command_inout_asynch("Init", true);

    if ( Tango::UNKNOWN != expected_post_init_state )
    {
      sync_wait_state(expected_post_init_state, tmo_sec);
    }
  }
...
private:
  ...
  //---------------------------------------------------------------------------
  PropertiesSnapshot m_prop_snapshots;
};
